...
export default function Footer() {
  ...
  const getInputStyle = (fieldName) => {
    const baseStyle = "w-full px-4 py-3.5 rounded-xl bg-white/5 text-white placeholder-gray-400 focus:outline-none transition-all duration-300 border";
  ...
  return (
    <footer className="relative w-full text-white overflow-hidden min-h-[550px]">
      ...
      {/* Main Content */}
      <div className="relative h-full max-w-7xl mx-auto px-6 py-10 flex flex-col justify-between">
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-start gap-3 p-4 bg-gradient-to-r from-white/5 to-transparent rounded-2xl border border-white/10 hover:border-orange-500/30 transition-all group">
              ...
              <p className="text-gray-300 text-sm leading-relaxed">
                Join <span className="text-orange-400 font-semibold">Designs Genie</span> and get a 
                <span className="text-white font-semibold"> FREE consultation</span> worth $299! 
            ...
            <div className="grid grid-cols-2 gap-3">
              <a href="mailto:leads@designsgenie.com" className="group">
                <div className="flex items-center gap-2 p-3 bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 hover:border-orange-500 hover:bg-orange-500/10 transition-all">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium group-hover:text-orange-300 transition-colors">leads@designsgenie.com</p>
                  </div>
                </div>
              </a>
              
              <a href="tel:+18723353070" className="group">
                <div className="flex items-center gap-2 p-3 bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 hover:border-orange-500 hover:bg-orange-500/10 transition-all">
                  <div className="w-8 h-8 bg-orange-500/20 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium group-hover:text-orange-300 transition-colors">(872) 335-3070</p>
                  </div>
                </div>
              </a>
            </div>
            ...
            <div className="flex items-center gap-4 pt-2">
              <div className="flex -space-x-2">
                {[1,2,3,4].map((i) => (
                  <div key={i} className="relative group/avatar">
                    <div className="w-7 h-7 rounded-full bg-gradient-to-br from-orange-400 to-red-400 border-2 border-white/30 flex items-center justify-center text-xs font-bold hover:scale-110 transition-transform cursor-pointer">
                      {String.fromCharCode(64 + i)}
                    </div>
                    <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 bg-gray-900 text-[8px] px-2 py-1 rounded opacity-0 group-hover/avatar:opacity-100 transition-opacity whitespace-nowrap">
                      Client {String.fromCharCode(64 + i)}
                    </div>
                  </div>
                ))}
                <div className="w-7 h-7 rounded-full bg-orange-500/30 border-2 border-white/30 flex items-center justify-center text-[10px] font-bold">
                  +100
                </div>
              </div>
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-gray-400">(4.9/5 from 200+ reviews)</span>
              </div>
            </div>
          </div>
    ...
    <div className="relative bg-gradient-to-br from-gray-900/95 to-gray-800/95 backdrop-blur-xl p-5 rounded-2xl border border-white/20">
      ...
      {submitStatus === 'success' && (
        <div className="mb-3 p-2 bg-green-500/20 border border-green-500/50 rounded-lg flex items-center gap-2 animate-slideDown">
          ...
          <p className="text-xs text-green-400">Thanks! We'll contact you soon.</p>
        </div>
      )}
      {submitStatus === 'error' && (
        <div className="mb-3 p-2 bg-red-500/20 border border-red-500/50 rounded-lg flex items-center gap-2 animate-slideDown">
          ...
          <p className="text-xs text-red-400">Failed. Please try again.</p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-2.5">
        ...
        <div className="flex items-center justify-center gap-1.5 pt-1">
          ...
          <p className="text-[8px] text-gray-500">100% secure. No spam.</p>
        </div>
      </form>
    </div>
  </div>
        ...
        <div className="relative mt-6 pt-4 border-t border-white/10">
          <div className="flex flex-col md:flex-row justify-between items-center gap-3">
            <p className="text-gray-400 text-[10px] flex items-center gap-2">
              <span>© 2026 Designs Genie</span>
              <span className="w-1 h-1 bg-gray-600 rounded-full"></span>
              <span className="text-gray-500">All rights reserved</span>
            </p>
            
            <div className="flex items-center gap-3">
              <span className="text-[8px] text-gray-600 tracking-wider">FOLLOW US</span>
              {[
                { icon: "F", color: "hover:bg-blue-600" },
                { icon: "T", color: "hover:bg-sky-500" },
                { icon: "L", color: "hover:bg-blue-700" },
                { icon: "IG", color: "hover:bg-pink-600" }
              ].map((social, i) => (
                <a
                  key={i}
                  href="#"
                  className="relative w-7 h-7 rounded-full bg-white/5 hover:bg-orange-500 border border-white/10 flex items-center justify-center text-[9px] text-gray-400 hover:text-white transition-all duration-300 hover:scale-110 group/social"
                >
                  <span className="relative z-10">{social.icon}</span>
                  <span className="absolute inset-0 rounded-full bg-gradient-to-r from-orange-500 to-red-500 opacity-0 group-hover/social:opacity-100 transition-opacity blur-md"></span>
                </a>
              ))}
            </div>
          </div>
